...
function handleSlide(e){
	...
	if(hasLost()){
		...
		setTimeout(()=>{
			alert("Game Over! TAMA NA! MAPAPAGOD KA LANG!");
			restartGame();
	...
	for(let r = 0; r<rows; r++){
		for(let c = 0; c<columns; c++){
			...
			if(board[r][c] == 2048 && is2048Exist == false ){
				alert("You Wind! You Got the 2048! BATAK KANA!")
				is2048Exist = true;
			}
			else if(board[r][c] == 4096 && is4096Exist == false){
				alert("You Win! You Got the 4096! TAMA NA BATAK KANA!")
				is4096Exist = true;
			}
			else if(board[r][c] == 8192 && is8192Exist == false){
				alert("You Wind! You Got the 8192! MASYADO KA NANG BATAK!")
				is8192Exist = true;
			}
		}
	}
